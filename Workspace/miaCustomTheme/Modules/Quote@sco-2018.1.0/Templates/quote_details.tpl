	...
	<div class="quote-details-view">
		<header>
			<h2 class="quote-details-header-title">
				{{#if isOpportunity}}{{translate 'Opportunity '}}{{else}}{{translate 'Quote '}}{{/if}}
		...
		<div class="quote-details-row">
			<div class="quote-details-content-col">
				{{#if isOpportunity}}
					<div class="opportunity_field_values">
					<label><b>Title:</b>{{model.title}}</label>
					<label><b>Estimated Budget: </b>{{model.estimatedbudget}}</label>
					<label><b>Budget Approved:</b>{{#ifEquals model.isbudgetapproved 'T'}}Yes{{else}}No{{/ifEquals}}</label>
					<label><b>Buying Time Frame: </b>{{model.buyingtimeframe}}</label>
					<label><b>Buying Reason:</b>{{model.buyingreason}}</label>
					<label><b>Sales Readiness: </b>{{model.salesreadiness}}</label>
					<label><b>Details:</b>{{model.memo}}</label>
					</div>
				{{/if}}
			...
			<div class="quote-details-summary-col">
				...
				<div class="quote-details-row-fluid">
					...
					{{/if}}
					{{#if isOpportunity}}
						{{#unless isQuoteCreated}}
						<a name="transform_to_quote" class="quote-details-button-download-pdf">Convert as Quote</a>
