<div class="order-history-summary-summary-col">
	...
	<div class="order-history-summary-row-fluid">
		...
		{{#if macUrl}}
			...
			<a href="{{macUrl}}" target="_blank" class="order-history-summary-button-download-pdf">
				{{translate 'Download MAC Address'}}
			</a>
		{{/if}}
		
		<!-- Follow Up -->
		<button name="follow_up_order" class="order-history-summary-button-download-pdf" data-id="{{model.internalid}}">
			{{translate 'FOLLOW UP'}}
		</button>
		...
		{{/if}}
		{{#if showViewCashSaleButton}}
			<a data-permissions="" href="/cashsales/{{cashsaleModel.internalid}}" data-id="{{cashsaleModel.internalid}}" class="order-history-summary-button-view-invoice">
				{{translate 'View Cash Sale'}}
			</a>
		{{/if}}
	</div>
</div>
