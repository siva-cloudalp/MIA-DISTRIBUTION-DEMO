...
<select name="is_drop_ship">
	...
	<option value="1" {{#if isDropShipYes}}selected {{/if}}>Yes</option>
	<option value="2" {{#if isDropShipNo}}selected {{/if}}>No</option>
</select>
...
<div class="order-wizard-dropship-module">
	...
	<select name="is_partial_shipment">
		<option value="T" {{#if isPartialShipment}}selected{{/if}}>Yes</option>
		<option value="F" {{#unless isPartialShipment}}selected{{/unless}}>No</option>
	</select>
					...
					{{#each shippingMethods}}
						...
						data-value="{{internalid}}">
							...
							<span class="order-wizard-shipmethod-module-option-name">{{name}}
								{{#unlessEquals internalid '15708'}}<span class="order-wizard-shipmethod-module-option-price">{{rate_formatted}}</span>{{/unlessEquals}}
							</span>
						</a>
					{{/each}}
				{{/if}}
