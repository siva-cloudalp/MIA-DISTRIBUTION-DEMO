<tr class="transaction-line-views-cell-selectable-actionable-navigable-row{{#if isLineChecked}} selected{{/if}}" {{#unless disableReturns}}data-action="{{actionType}}"{{/unless}} data-id="{{lineId}}">
	<td class="transaction-line-views-cell-selectable-actionable-navigable-select">
		<input type="checkbox" value="{{itemId}}" data-action="select" {{#if isLineChecked}}checked{{/if}} {{#if disableReturns}}disabled{{/if}}>
	</td>
	...
	<td class="transaction-line-views-cell-selectable-actionable-navigable-actions">
		...
		{{#if disableReturns}}this item is not returnable{{/if}}
	</td>
</tr>
