...
class HelpDisplay {
	...
	displayHelp() {
		...
		console.log(color.bold('Usage: gulp [task] [options]'));
		console.log(color.bold('Tasks:'));
		Object.keys(help).forEach((group) => {
			console.log(groupMinIdentation + color.bold(group));
			...
			help[group].forEach((task) => {
				...
				console.log(
					`${taskMinIdentation + color.bold.cyan(task.name.content)} ${color.bold(
				...
				task.args.forEach((arg) => {
					...
					console.log(
						`${argMinIdentation}${color.bold.green(argPrefix + arg.content)}${identation + arg.description}`
					);
				});
				// line break
				console.log();
