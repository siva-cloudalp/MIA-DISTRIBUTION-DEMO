...
module.exports = class extends ModuleGenerator {
  ...
  prompting() {
    ...
    return this.prompt([
      ...
      },
      {
        ...
        name: 'target',
        message:
          'This extension supports (Press ' + c.cyan('<space>') + ' to select)',
        default: ['SCS'],
        choices: [
  ...
  configuring() {
    ...
    this.log(
      c.green(
        `\nGood! The process "extension:create" finished after ${Math.round(
          time_diff / 1000,
        )} sec.`,
      ),
    );
    ...
    if (this.is_scis) {
      this.log(
        `${c.green(
          '\t1- ',
        )}gulp extension:fetch - to get the initial setup files and optionally start developing from extensions in the file cabinet.\n${c.green(
          '\t2- ',
        )}gulp extension:update-manifest - update the manifest.json according to the files you added or removed.\n${c.green(
          '\t3- ',
        )}gulp extension:local - (Optional) to develop locally using javascript.\n${c.green(
          '\t4- ',
      ...
      this.log(
        `${c.green(
          ...
          '\t4- ',
        )}gulp extension:local - (Optional) to develop locally styles, template and javascript files.\n${c.green(
          '\t5- ',
        )}gulp extension:deploy - to deploy your extension to the file cabinet and try it using the Extension Management UI.\n`,
      );
    }
    ...
    this.log(
      '\tType ' + c.cyan('gulp') + ' for extra help on how to use the commands.',
    );
  }
};
